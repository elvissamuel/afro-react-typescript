...
export default function DialogueBox(props: Props) {
  ...
  const handleDelete = async (id: string) =>{
    const data = {product_id: id, authorization: user?.authorization};
    const encryptedInfo = encryptData({data, secretKey: process.env.REACT_APP_AFROMARKETS_SECRET_KEY })
    const response: any = await deleteProduct(encryptedInfo)
    if (response.data) {

      if(response.status === 200){
      await decryptAES(response.data, process.env.REACT_APP_AFROMARKETS_SECRET_KEY);
  ...
  return (
    <Transition show={props.openForm}>
      <Dialog className="relative z-10" onClose={props.setOpenForm}>
        <TransitionChild
              ...
              <DialogPanel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                ...
                <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                  <button
                    ...
                    onClick={() => handleDelete(props.product.productId)}
                  >
                    Delete
                  </button>
                  <button
